...
class MainActivity : AppCompatActivity() {
    ...
    private fun showResult(title: String, message: String){
        val alertDialog: AlertDialog = this.let {
            ...
            builder.apply {
                setPositiveButton("OK") { dialog, _ -> resetAnswer()
                    result = 0
                    dialog.dismiss()
                }
            }
    ...
    fun submit(view: View) {
        ...
        val message = if(result > 0)
            "Congratulations! You submitted on $date and $time, Your achieved $result%"
